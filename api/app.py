...
async def get_config(
    ...
    if rua_values and any(rua in user_agent for rua in rua_values.split(',')):
        errormsg = orjson.dumps({'status': 'error', 'message': 'block'}, option=orjson.OPT_INDENT_2)
        return Response(content=errormsg, status_code=403, media_type="application/json")
    ...
    try:
        selected_template_index = int(file) if file and file.isdigit() else 0
        current_config['config_template'] = normalize_scheme(file) if file else ''
