...
def config(url):
    ...
    if rua_values and any(rua_value in user_agent for rua_value in rua_values.split(',')):
        return Response(json.dumps({'status': 'error', 'message': 'block'}, indent=4, ensure_ascii=False),content_type='application/json; charset=utf-8', status=403)

    query_string = request.query_string.decode('utf-8')
    full_raw_input = f"{url}?{query_string}" if query_string else url
    full_raw_input = unquote(full_raw_input)
    ...
    special_keys = ['emoji', 'file', 'eps', 'enn', 'prefix', 'suffix', 'tag', 'ua', 'UA', 'gh', 'onlynodes', 'subgroup']
    pattern = re.compile(r'&(' + '|'.join(special_keys) + r')=')

    match = pattern.search(full_raw_input)
    parsed_params = {}

    if match:
        split_index = match.start()
        full_url_part = full_raw_input[:split_index] 
        param_str = full_raw_input[split_index + 1:] 
        for item in param_str.split('&'):
            if '=' in item:
                k, v = item.split('=', 1)
                parsed_params[k] = v
    else:
        full_url_part = full_raw_input

    def get_param(name):
        return parsed_params.get(name, '')
    ...
    UA_param = get_param('UA')
    prefix_param = get_param('prefix')
    suffix_param = get_param('suffix')
    exclude_protocol_param = get_param('eps')
    exclude_node_name_param = get_param('enn')
    gh_proxy_param = get_param('gh')
    onlynodes_param = get_param('onlynodes')
    subgroup_param = get_param('subgroup')

    full_url = normalize_scheme(full_url_part)

    if '/api/v4/projects/' in full_url:
        parts_api = full_url.split('/api/v4/projects/')
        full_url = parts_api[0] + '/api/v4/projects/' + parts_api[1].replace('/', '%2F', 1)

    url_parts = full_url.split('|')
    emoji_list = emoji_param.split('|') if emoji_param else []
    tag_list = tag_param.split('|') if tag_param else []
    final_ua_source = UA_param if UA_param else ua_param
    ua_list = final_ua_source.split('|') if final_ua_source else []
    prefix_list = prefix_param.split('|') if prefix_param else []
    suffix_list = suffix_param.split('|') if suffix_param else []
    subgroup_list = subgroup_param.split('|') if subgroup_param else []

    file_val = normalize_scheme(file_param)

    existing_subs = temp_json_data.get('subscribes', [])
    first_template = existing_subs[0] if existing_subs else {}

    new_subs = []
    for idx in range(len(url_parts)):
        sub = existing_subs[idx].copy() if idx < len(existing_subs) else first_template.copy()
        sub['url'] = normalize_scheme(url_parts[idx])
        sub['prefix'] = prefix_list[idx] if len(prefix_list) > idx else ''
        sub['suffix'] = suffix_list[idx] if len(suffix_list) > idx else ''
        sub['tag'] = tag_list[idx] if len(tag_list) > idx else sub.get('tag','')
        sub['User-Agent'] = ua_list[idx] if (len(ua_list) > idx and ua_list[idx]) else 'v2rayng'
        sub['subgroup'] = subgroup_list[idx] if len(subgroup_list) > idx else ''
        sub['ex-node-name'] = exclude_node_name_param if exclude_node_name_param else ''
        sub['enabled'] = True
        try:
            sub['emoji'] = int(emoji_list[idx]) if len(emoji_list) > idx and emoji_list[idx].isdigit() else sub.get('emoji', 1)
        except:
            sub['emoji'] = 1
        new_subs.append(sub)

    temp_json_data['subscribes'] = new_subs
    temp_json_data['exclude_protocol'] = exclude_protocol_param if exclude_protocol_param else temp_json_data.get('exclude_protocol', '')
    temp_json_data['config_template'] = file_val if file_val and not file_val.isdigit() else temp_json_data.get('config_template', '')
    temp_json_data['Only-nodes'] = True if onlynodes_param == '1' else False
    ...
    try:
        ...
        if gh_proxy_param:
            selected_gh_proxy_index = str(int(gh_proxy_param) - 1) if gh_proxy_param.isdigit() else gh_proxy_param
        ...
        if CONFIG_FILE_NAME.startswith("./"): CONFIG_FILE_NAME = CONFIG_FILE_NAME[2:]

        os.environ['TEMP_JSON_DATA'] = json.dumps(json.loads(data_json['TEMP_JSON_DATA']), indent=4, ensure_ascii=False)

        config_file_path = os.path.join('/tmp/', CONFIG_FILE_NAME)
        if not os.path.exists(config_file_path): config_file_path = CONFIG_FILE_NAME
...
def generate_config():
    try:
        ...
        # return Response(config_content, content_type='application/json; charset=utf-8',headers={"Content-disposition": f"attachment; filename={CONFIG_FILE_NAME}"})
