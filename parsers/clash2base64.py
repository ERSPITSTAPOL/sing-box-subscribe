...
def clash2v2ray(share_link):
    ...
    if share_link['type'] == 'vmess':
        ...
        if vless_info['type'] == 'ws':
            ...
            link = "vless://{uuid}@{server}:{port}?encryption=none&security={security}&sni={sni}&fp={fp}&type={type}&host={host}&path={path}&flow={flow}&alpn={alpn}&allowInsecure={allowInsecure}".format(**vless_info)
