...
def get_content_from_url(url, n=10):
    ...
    def internal_recognize(current_response):
        ...
        if re.fullmatch(r'^[A-Za-z0-9+/=_ \-]+$', response_text):
            ...
            yaml_content = current_response.content.decode('utf-8')
    ...
    if not main_text_check:
        response = tool.getResponse(url, custom_user_agent='clash.meta')
...
def combin_to_config(config, data):
    has_template_func = 'pro_node_template' in globals()
    template_func = globals().get('pro_node_template') if has_template_func else None
    ...
    for node in nodes.values():
        ...
        final_resolved = []
        seen = set()
        for tag in temp_resolved:
            if tag in seen: continue
            seen.add(tag)
            if tag not in nodes and tag in empty_data_keys:
                continue
            final_resolved.append(tag)
            if tag in nodes:
                reverse_dep[tag].add(node['tag'])
        node['resolved'] = final_resolved
    empty_queue = deque([n['tag'] for n in nodes.values() if not n['resolved']])
    alive = {n['tag']: bool(n['resolved']) for n in nodes.values()}
    while empty_queue:
    ...
    for node in nodes.values():
        if alive.get(node['tag']):
            node['obj']["outbounds"] = node['resolved']
            node['obj'].pop("filter", None)
            out.append(node['obj'])
    out.extend([item for group in data if data[group] for item in data[group]])
    ...
    if config.get('dns') and config['dns'].get('servers'):
        ...
        if asod and asod.get('proxy') and asod.get('direct'):
             if asod['proxy'] in dns_tags and asod['direct'] in dns_tags:
                if 'set_proxy_rule_dns' in globals(): set_proxy_rule_dns(config)

    wireguard_items = [item for item in config['outbounds'] if item.get('type') == 'wireguard']
    if wireguard_items:
        endpoints = list(wireguard_items)
        new_config = OrderedDict()
        for key, value in config.items():
            new_config[key] = value
            if key == 'outbounds': new_config['endpoints'] = endpoints
        config = new_config
